...
use UserBundle\Mapping\MappedEventSubscriber;
...
abstract class AbstractTrackingListener extends MappedEventSubscriber
{
            ...
            if ($uow->isScheduledForInsert($object) && isset($config['create'])) {
                foreach ($config['create'] as $field) {
                    ...
                    if ($new === null) { // let manual values
                        $needChanges = true;
                        $this->updateField($object, $ct, $co, $ea, $meta, $field);
                    }
                }
            }
                    ...
                    foreach ($trackedFields as $tracked) {
                        ...
                        if (isset($changeSet[$tracked])) {
                            ...
                            if (($singleField && in_array($value, (array) $options['value'])) || $options['value'] === null) {
                                $needChanges = true;
                                $this->updateField($object, $ct, $co, $ea, $meta, $options['field']);
                            }
                        }
                    }
                }
     ...
     */
    abstract protected function getFieldValue($meta, $field, $container, $context, $eventAdapter);
     ...
     * @param string             $field
     */
    protected function updateField($object, $container, $context, $eventAdapter, $meta, $field)
    {
        /** @var \Doctrine\Orm\Mapping\ClassMetadata|\Doctrine\ODM\MongoDB\Mapping\ClassMetadata $meta */
        $property = $meta->getReflectionProperty($field);
        $oldValue = $property->getValue($object);
        $newValue = $this->getFieldValue($meta, $field, $container, $context, $eventAdapter);
